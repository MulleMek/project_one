...
class MySqlDB {
  ...
  public function isTableExist($tableName){
    $pre = $this->_mysql->query("SHOW TABLES LIKE '" . $tableName . "'");
    $result = $pre->num_rows;
    return (bool) $result;
  }
   ...
   {
      //dd('bad bad');  /// on delete in modifiers
      if(!$result = $this->_mysql->query($query)){
       
        return false;
        //die('There was an error running the query [' . $this->_mysql->error . ']');
      }
      return true;
   }
    ...
    * @return bool
    */
   public function unsafeSelect($query) 
   {
      if(!$result = $this->_mysql->query($query, MYSQLI_USE_RESULT)){
        //return false;
        die('There was an error running the query [' . $this->_mysql->error . ']');
      }
    ...
    */
   public function query($query, $good = true) 
   {
      $this->_query = ($good)? filter_var($query, FILTER_SANITIZE_STRING) : $query;
    ...
    // нужный параметр будет ключём, работает только с запросами select
    // предполагаеся использовать ключ
    public function iquery($query, $param)
    {
      $this->_query = $query;
      $stmt = $this->_prepareQuery();
      $stmt->execute();
    ...
    * @param <string $tableName The name of the table.
    * @param array $insertData Data containing information for inserting into the DB.
    * @return inserted ID or Zero
    */
   public function insertReturnInsertedId($tableName, $insertData) 
   {
      $this->_query = "INSERT into `$tableName`";
      $stmt = $this->_buildQuery(NULL, $insertData);
      $stmt->execute();
   ...
   protected function _buildQuery($numRows = NULL, $tableData = false) 
   {
      ...
      if (!empty($this->_where)) {
         ...
         $where_values = array_values($this->_where);
