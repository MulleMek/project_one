...
class PJLogger 
{
	...
	{
		//определяем куда сохраним логи
		$dir = null;
		// Клиент
		if($path == 1) $dir = PROJECT."/Logger/Client/";		
		// Сервер
		if($path == 2) $dir = PROJECT."/Logger/Server/";
		// Нанокасса
		if($path == 4) $dir = PROJECT."/Logger/Nanokassa/";
		// дефолтные настройки
		if($path == 0 || !$dir) $dir = PROJECT."/Logger/Custom/";
		...
		if ($handle = opendir($dir)) {
			while (false !== ($file = readdir($handle))) { 
				...
				if(!in_array($file, [".","..", ".gitkeep"])){
					...
					if (intval(time())-intval(strtotime($_file)) > 14*24*60*60 ) {
						unlink($dir.$file);
					}
				}
			}
